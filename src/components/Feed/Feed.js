    ...
    let postsToDisplay = this.state.post.map((element, index) => {
      ...
      if (this.props.userReducer.user.user_id === element.user_id) {
        return (
          <div key={index}>
            <h3> {element.post} </h3>
            {this.props.userReducer.user.user_id === element.user_id && (
              <button
                onClick={() =>
                  this.deletePost(element.user_id, element.post_id)
                }
              >
                X
              </button>
            ...
            {(!this.state.editInput && (
              <i
                ...
                onClick={() =>
                  this.handleFlag(this.state.editInput, element.post_id, "")
                }
              />
            )) ||
              (this.state.editInput &&
                this.state.currentEditId === element.post_id && (
                  <div>
                    <input
                      placeholder="Insert New Post Here"
                      onChange={e =>
                        this.handleFlag(
                          !this.state.editInput,
                          element.post_id,
